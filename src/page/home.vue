<template>
    <div>
        ...
        <scroll-view>
            <div :style="{visibility:showMain}" class="main">
                <router-link ref="a" v-for="(msg,index) in list" v-bind:key="index" :to="{name:'chatPage',query:{title:msg.username}}">
                    <line-message-index :msg="msg"></line-message-index>
                </router-link>
                <loading :loading="isShowBottomLoading"></loading>
            </div>
        </scroll-view>
        <full-loading :full-loading="isShowLoading"></full-loading>
        <footer-nav active='home'></footer-nav>
    </div>
</template>  
